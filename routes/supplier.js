...
app.put('/updateSupplierPaymentsAndPurchaseOrder', (req, res) => {
  ...
  db.query(updateSubConPaymentsQuery, [supplier_receipt_id], (err, result1) => {
    if (err) {
    ...
    db.query(updateSubConWorkOrderQuery, [purchase_order_id], (err, result2) => {
      if (err) {
...
app.post('/getPurchaseOrderLinkedss', (req, res, next) => {
  db.query(`SELECT p.*
    ...
    FROM supplier_receipt_history supHist 
    LEFT JOIN supplier_receipt r ON (r.supplier_receipt_id = supHist.supplier_receipt_id) 
    WHERE supHist.purchase_order_id = p.purchase_order_id AND r.receipt_status != 'Cancelled' ) as prev_amount 
  FROM purchase_order p
