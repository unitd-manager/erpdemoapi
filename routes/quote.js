  ...
  db.query(
    ...
    (err, result) => {
      if (err) {
        ...
        res.status(500).send({ msg: 'Error fetching quotation' });
        return;
      }
      ...
      if (result.length > 0) {
        ...
        res.status(404).send({ msg: 'No quotation found' });
      }
    }
  );
});
// app.post('/getQuotationCounts', (req, res, next) => {
//   const { month } = req.query; 
//   db.query(
//     `SELECT COUNT(*) AS quotation_count
//   FROM quote
//   WHERE quote_date = =${db.escape(month)}`,
//   (err, result) => {
//     if (err) {
//       console.log("error: ", err);
//       return;
